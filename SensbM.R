  ...
  
{
  ...
  # Second Population
  SM2[i+1]=SM2[i]+h1*(((bM2[i]*NM2[i]-(mM1[i])*SM2[i])*(1-SM2[i]/KB))-(c2[i]*SM2[i]*IBC2[i])/KB-(c1[i]*SM2[i]*IBSC2[i])/KB)
  ...
  # Mosquito Population Loop
  # Third Population
  SM3[i+1]=SM3[i]+h1*(((bM3[i]*NM2[i]-(mM1[i])*SM3[i])*(1-SM3[i]/KB))-(c2[i]*SM3[i]*IBC3[i])/KB-(c1[i]*SM3[i]*IBSC3[i])/KB)
...
results1 <- data.frame(R10,t1/365);
results1$Group <-"bM1"
names(results1) <- c("R0","Time","Group")

resinf1<- data.frame(SM1, EM1, IM1, IBC1, IBSC1,t1/365)
resinf1$Group <-"bM1"
names(resinf1) <- c("SM","EM","IM","IBC","IBSC","Time","Group")

results2 <- data.frame(R20,t1/365);
results2$Group <-"bM2"
names(results2) <- c("R0","Time","Group")

resinf2<- data.frame(SM2, EM2, IM2, IBC2, IBSC2,t1/365)
resinf2$Group <-"bM2"
names(resinf2) <- c("SM","EM","IM","IBC","IBSC","Time","Group")

results3 <- data.frame(R30,t1/365);
results3$Group <-"bM3"
names(results3) <- c("R0","Time","Group")

resinf3<- data.frame(SM3, EM3, IM3, IBC3, IBSC3,t1/365)
resinf3$Group <-"bM3"
names(resinf3) <- c("SM","EM","IM","IBC","IBSC","Time","Group")

df1 <-data.frame(resinf1,resinf2,resinf3)
...
ggsave("bMImpactR0.eps", width = 17, height = 17, units = "cm")

#dev.off()
######
ggplot(df1)+
  theme_bw()+
  geom_line(aes(x=df1$Time, y=df1$IM, color = "red"),size=2)+
  geom_line(aes(x=df1$Time, y=df1$IM.1, color = "blue"),size=2)+
  geom_line(aes(x=df1$Time, y=df1$IM.2, color = "black"),size=2)+
  theme(axis.title = element_text(color="black", face="bold", size=12))+
  theme(legend.title=element_blank())+
...
ggplot(df)+
  theme_bw()+
  geom_line(aes(x=t1, y=(R10/10), color = "red"),size=1)+
  geom_line(aes(x=t1, y=(R20/10), color = "blue"),size=1)+
  geom_line(aes(x=t1, y=(R30/10), color = "green"),size=1)+
  theme(axis.title = element_text(color="black", face="bold", size=12))+
  theme(legend.title=element_blank())+
  labs(x = "Time", y= expression(paste("", R[0])), size = 20)+
  theme(legend.background = element_rect())+
